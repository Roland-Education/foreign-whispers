...
class TTSService:
    ...
    def text_file_to_speech(
        ...
        *,
        ...
        Args:
            source_path: Path to the translated transcript JSON.
            output_path: Directory where the rendered WAV is written.
            alignment: True/False to force aligned/baseline mode, or None to
                read the FW_ALIGNMENT environment variable.
            speaker_wav: Default reference voice (relative to the speakers
                volume) used when a segment has no entry in *voice_map*.
            voice_map: Mapping from speaker label (e.g. ``"SPEAKER_00"``)
                to reference WAV path. When provided, segments are
                synthesised with the speaker-specific voice.
        """
